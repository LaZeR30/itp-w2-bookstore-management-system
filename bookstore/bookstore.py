def create_bookstore(name):
    bookstore = {'name': name, 'books': [], 'authors': []}
    return bookstore

def get_bookstore_name(bookstore):
    return bookstore['name']

def add_author(bookstore, name, nationality):
    author = {
        'name': name,
        'nationality': nationality
    }
    bookstore['authors'].append(author)  # we have a dict.key which is a List - we're append to List 
    return author

def get_author_by_name(bookstore, name):
    for author in bookstore['authors']:
        if author['name'] == name:
            return author

def add_book(bookstore, title, isbn, author):
     # first we need a dict
    book = {'title': title, 'isbn': isbn, 'author': author}
    # add book to bookstore
    bookstore['books'].append(book)
    
def get_book_by_title(bookstore, title):
    for book in bookstore['books']:
        if book['title'] == title:
            return book

def get_books_by_author(bookstore, author):
    # create a list of books and return book
    books = []
    for book in bookstore['books']:
        if book['author'] == author:
            books.append(book)
    
    return books
        
